const seedGames = [
  {id: 1, name: 'Nebula Protocol', platform: 'PS5', category: 'Ação', condition: 'Novo', price: 249.9, stock: 5, icon: 'NP'},
  {id: 2, name: 'Circuit Breakers', platform: 'PC', category: 'Corrida', condition: 'Novo', price: 89.9, stock: 8, icon: 'CB'},
  {id: 3, name: 'Moon Harbor', platform: 'Switch', category: 'Aventura', condition: 'Usado', price: 119.9, stock: 2, icon: 'MH'},
  {id: 4, name: 'Arcade Archive 92', platform: 'Retro', category: 'RPG', condition: 'Usado', price: 69.9, stock: 3, icon: 'AA'},
  {id: 5, name: 'Crystal Drift', platform: 'Xbox', category: 'Esporte', condition: 'Novo', price: 199.9, stock: 4, icon: 'CD'},
];
...
function renderLogin() {
  ...
  root.innerHTML = `
    <div class="login">
      <form id="login-form">
        ...
        <p class="kicker">BEM-VINDO</p>
        <h2>${form.mode === 'admin' ? 'Acesso Administrativo' : 'Acesse sua conta'}</h2>
        
        <div class="switch">
          <button type="button" class="${form.mode === 'client' ? 'on' : ''}" id="btn-mode-client">Cliente</button>
          <button type="button" class="${form.mode === 'admin' ? 'on' : ''}" id="btn-mode-admin">Administrador</button>
        </div>

        ${form.mode === 'client' ? `<input id="log-name" placeholder="Seu nome" value="${form.name}"/>` : ''}
        <input id="log-email" type="email" placeholder="E-mail" value="${form.email}"/>
        ${form.mode === 'admin' ? `<input id="log-pass" type="password" placeholder="Senha" value="${form.password}"/>` : ''}
        
        ${form.error ? `<p class="error">${form.error}</p>` : ''}
        
        <button class="action" type="submit">${form.mode === 'admin' ? 'Entrar no Painel' : 'Entrar na Loja'}</button>
  ...
  document.getElementById('login-form').onsubmit = (e) => {
    ...
    if (form.mode === 'admin' && (form.email !== 'admin@fozgames.com' || form.password !== 'admin123')) {
      setState({ loginForm: { ...form, error: 'As credenciais de administrador estão incorretas.' } });
      return;
    }
    if (form.mode === 'client' && (!form.name.trim() || !form.email.trim())) {
      setState({ loginForm: { ...form, error: 'Informe seu nome e e-mail para continuar.' } });
      return;
    }
    setState({
      user: {
        name: form.mode === 'admin' ? 'Administrador da loja' : form.name,
        email: form.email,
        role: form.mode
      },
      loginForm: { mode: 'client', name: '', email: '', password: '', error: '' }
...
function createHeader() {
  ...
  header.innerHTML = `
    ...
    <nav>
      <button class="${state.view === 'store' ? 'on' : ''}" id="nav-store">Loja</button>
      ...
      <button class="profile">
        ...
        ${state.user.name}
        <small>${state.user.role === 'admin' ? 'Administrador' : 'Cliente'}</small>
      </button>
      <button class="signout" id="nav-logout">Sair</button>
    </nav>
  `;
...
function createStore() {
  ...
  sec.innerHTML = `
    <div class="hero">
      <p>FOZGAMES</p>
      <h1>Jogue algo<br/><em>inesperado.</em></h1>
      <span>Jogos escolhidos a dedo, prontos para uma nova casa.</span>
    </div>
    <div class="filterbar">
      <div>
  ...
  filtered.forEach(game => {
    ...
    article.innerHTML = `
      ...
      <div class="tilecopy">
        ...
        <h2>${game.name}</h2>
        <span>${game.stock} un. em estoque</span>
        <strong>${money(game.price)}</strong>
        <button id="add-${game.id}">Adicionar ao Carrinho</button>
      </div>
    `;
...
function createCart() {
  ...
  if (state.cart.length === 0) {
    sec.innerHTML = `
      ...
      <div class="empty">
        ...
        <button id="btn-back-store">Voltar para a loja</button>
      </div>
    `;
  ...
  sec.innerHTML = `
    ...
    <div class="checkout">
      <h2>Total do Pedido <b>${money(cartTotal)}</b></h2>
      <div class="fields">
        <input id="chk-address" placeholder="Endereço de entrega" value="${state.cartForm.address}"/>
        <input id="chk-phone" placeholder="Celular (WhatsApp)" value="${state.cartForm.phone}"/>
      </div>
      <button class="action" id="btn-finish">Finalizar Compra</button>
      <small>Seus dados estão seguros. Entraremos em contato via WhatsApp para confirmar o pagamento.</small>
    </div>
  `;
  ...
  sec.querySelector('#btn-finish').onclick = () => {
    if (!state.cartForm.address.trim() || !state.cartForm.phone.trim()) {
      showNotice('Informe o endereço de entrega e celular.');
      return;
    }
    const order = {
      id: Date.now(),
...
function createAdmin() {
  ...
  sec.innerHTML = `
    ...
    <div class="adminlayout">
      <form id="add-game-form">
        ...
        <input id="frm-icon" placeholder="Iniciais (máx 4)" maxlength="4" value="${form.icon}"/>
        
        <label class="upload">
          ${form.image ? `<img src="${form.image}" alt="Prévia"/>` : `<span>Adicionar Imagem<br/><small>JPG, PNG, WEBP - max 1.5MB</small></span>`}
        ...
        <div class="pair">
          ...
          <input type="number" min="1" id="frm-stock" value="${form.stock}" placeholder="Quantidade"/>
        </div>
        
        <input type="number" step="0.01" id="frm-price" placeholder="Preço (R$)" value="${form.price}"/>
        <button class="action" type="submit">Salvar Produto</button>
      </form>
      
      <div class="inventory">
        <h2>Produtos Cadastrados <span>${state.games.length} itens</span></h2>
  ...
  sec.querySelector('#frm-img').onchange = (e) => {
    ...
    if (file.size > 1500000) {
      showNotice('A imagem excede o limite de tamanho (1.5 MB).');
      return;
    }
...
function createSales() {
  ...
  if (!state.orders.length) {
    sec.innerHTML = `
      ...
      <div class="statrow">
        <div><small>TOTAL DE PEDIDOS</small><b>0</b></div>
        <div><small>RECEITA TOTAL</small><b>${money(0)}</b></div>
      </div>
      <div class="empty">
        <h2>Ainda não há pedidos.</h2>
        <p>As novas vendas aparecerão aqui.</p>
      </div>
    `;
    return sec;
  }
  ...
  sec.innerHTML = `
    ...
    <h1>Pedidos Recentes.</h1>
    <div class="statrow">
      <div><small>TOTAL DE PEDIDOS</small><b>${state.orders.length}</b></div>
      <div><small>RECEITA TOTAL</small><b>${money(total)}</b></div>
    </div>
    ...
    article.innerHTML = `
      ...
      <div>
        ...
        <p>
          ...
          ${order.items.map(item => `<span>${item.qty}x ${item.name} - ${money(item.price * item.qty)}</span>`).join('')}
        </p>
      </div>
    `;
